...
export const rankTracker = async(keyword, targetDomain) =>{
    ...
    try {
        ...
        let found = null,
        allResults = [];
        const clearTarget = targetDomain.toLowerCase().replace("www.","")

        // 3. Search Loop: Iterate through up to 5 pages fo Google results
                ...
                pageResults = await page.evaluate(()=> Array.from(document.querySelectorAll('h3')).map((h3)=>{
                    ...
                    if(!a){
                        ...
                        for(let j=0; j < 5 && p; j++, p=p.parentElement){
                            if(p.tagName === "A" && p.href && p.href.includes("url=")){
                                a = p;
                                break;
                            }
        ...
        for (const r of pageResults){
            ...
            allResults.push(r);
            if(!found && (r.domain.toLowerCase().includes(clearTarget) || clearTarget.includes(r.domain.toLowerCase()))){
        ...
        await browser.close();
        const competitors = allResults.filter((r)=> !r.domain.toLowerCase().includes(clearTarget) && !clearTarget.includes(r.domain.toLowerCase())).slice(0,10);
        return {
            success: true, 
    data: {
        keyword,
        ...
        console.log("Error in rankTracker service:", error.message);
