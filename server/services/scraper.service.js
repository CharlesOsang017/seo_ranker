...
export const scrapeUrl = async (url)=>{
    ...
    try {
        ...
        try {
            ...
            browser = null;
            return {status: "failed", error: navError.message}
        ...
        const scrapedData = await page.evaluate(()=>{
           ...
           allLinks.forEach((link)=>{
            try {
                ...
                if(href.startwith("mailto:") || href.startsWith("tel:")) return;
        ...
        return {
            success: true,
            data: {...scrapedData, statusCode, url, loadTime}
        }
    } catch (error) {
        console.error("[SCRAPER]: Failed to scrape ", error.message);
        if(browser){
            try {
                await browser.close().catch(()=> {});
            } catch (closeError) {
